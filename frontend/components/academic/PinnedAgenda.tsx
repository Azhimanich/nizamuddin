...
interface PinnedAgendaProps {
  locale: string
  schoolType?: 'tk' | 'sd' | 'smp' | 'kepondokan'
  maxItems?: number
}

export function PinnedAgenda({ locale, schoolType = 'tk', maxItems = 3 }: PinnedAgendaProps) {
  ...
  useEffect(() => {
    const fetchPinnedEvents = async () => {
      try {
        ...
        // Filter only pinned events and limit to maxItems
        const pinnedEvents = Array.isArray(data) 
          ? data.filter((event: any) => event.is_pinned).slice(0, maxItems)
          : []
